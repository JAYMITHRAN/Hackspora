...
import { API_BASE_URL, EDUCATION_LEVELS, INTEREST_CATEGORIES } from "@/lib/utils/constants"
...
export default function AssessmentPage() {
  ...
  const [personalInfo, setPersonalInfo] = useState({
    name: formData.preferences?.name || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const recommendedRole = response?.llmResponse?.recommendedRole
      if (recommendedRole) {
        localStorage.setItem("selected-career", JSON.stringify(recommendedRole))
      }
      ...
      <div className="grid gap-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="text-xs text-gray-600 space-y-1">
            ...
            <div>Local Llama endpoint: {`${API_BASE_URL}/api`}</div>
          </div>
        </div>
      </div>
    </div>
